...
RSpec.describe User, type: :model do
  describe 'Validations' do
    before :each do
      @user = User.new({ name: 'Who', email: 'whodis@email.com', password: '12345678', password_confirmation: '12345678', last_name: 'Dis' })
    end
    ...
    it 'must check email for uniquness regardless of cases' do
      @user.save
      @user2 = User.new ({ name: 'Who', email: 'WHODIS@EMAIL.COM', password: '1234', password_confirmation: '1234', last_name: 'Dis' })
      @user2.save
  ...
  describe '.authenticate_with_credentials' do
    ...
    it 'should be able to remove leading and trailing white space in email address' do
      expect(User.authenticate_with_credentials(' whodis@email.COM  ', @user.password)).to eq @user
    end
